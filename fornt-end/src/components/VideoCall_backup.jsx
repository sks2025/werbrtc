...
import LiveVideoUploader from './LiveVideoUploader';
...
const VideoCall = () => {
  ...
  // Removed old recording state - using LiveRecorder component now
  ...
  // Removed auto-recording - using LiveRecorder component for manual control
  ...
  const initializeCall = async () => {
    try {
      ...
      // Removed old recording handlers - using LiveRecorder component now
  ...
  const saveRecordingToDatabase = async (blob) => {
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await fetch('http://localhost:3002/api/media/save-recording', {
          method: 'POST',
          headers: {
  ...
  const saveImageToDatabase = async (imageData, fileName) => {
    try {
      const response = await fetch('http://localhost:3002/api/media/capture-image', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const saveSignature = async () => {
    ...
    if (signatureRef.current) {
      ...
      try {
        const response = await fetch('http://localhost:3002/api/media/save-signature', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <div className="video-call">
      ...
      {/* Live Recorder Component - Socket Only */}
      {role === 'doctor' && (
        <LiveRecorder 
          roomId={roomId}
          userInfo={userInfo}
      ...
      {/* Live Video Uploader Component - Multer Upload */}
      {role === 'doctor' && (
        <LiveVideoUploader 
          roomId={roomId}
          userInfo={userInfo}
                      ...
                      <div className="media-list">
                        {roomMediaData.recordings.map((recording, index) => (
                          <div key={index} className="media-item recording-item">
                            ...
                            {recording.filePath && (
                              <a 
                                href={`http://localhost:3002${recording.filePath}`}
                                target="_blank"
                                rel="noopener noreferrer"
